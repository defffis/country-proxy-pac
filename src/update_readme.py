...
COUNTRY_FLAGS = {"TR": "🇹🇷", "IN": "🇮🇳", "PL": "🇵🇱", "NL": "🇳🇱", "DE": "🇩🇪", "US": "🇺🇸", "GB": "🇬🇧"}
COUNTRY_NAMES_RU = {"TR": "Турция", "IN": "Индия", "PL": "Польша", "NL": "Нидерланды", "DE": "Германия", "US": "США", "GB": "Великобритания"}
COUNTRY_NAMES_EN = {"TR": "Turkey", "IN": "India", "PL": "Poland", "NL": "Netherlands", "DE": "Germany", "US": "United States", "GB": "United Kingdom"}
...
def build_stats_block(stats: dict) -> str:
    ...
    for code in COUNTRY_NAMES_RU:
        ...
        lines.append(
            f"| {COUNTRY_FLAGS[code]} {COUNTRY_NAMES_RU[code]} (`{code}`) | {fmt(c.get('working', 0))} | {fmt(c.get('http', 0))} | {fmt(c.get('https', 0))} | {fmt(c.get('socks4', 0))} | {fmt(c.get('socks5', 0))} | {fmt(c.get('pac_http_https', 0))} | {fmt(c.get('socks_pac', 0))} | {latency_text} |"
        )
    ...
    for code, name in stats.get("target_countries", {}).items():
        c = countries.get(code, {})
        lines.append(f"| `{name}.pac` | {fmt(c.get('pac_http_https', 0))} | [`{name}.pac`](https://raw.githubusercontent.com/defffis/country-proxy-pac/main/pac/{name}.pac) |")
        lines.append(f"| `{name}-socks.pac` | {fmt(c.get('socks_pac', 0))} | [`{name}-socks.pac`](https://raw.githubusercontent.com/defffis/country-proxy-pac/main/pac/{name}-socks.pac) |")
    ...
    for code in COUNTRY_NAMES_EN:
        ...
        latency = c.get("best_latency_ms")
        latency_text = f"{fmt(latency)} ms" if latency is not None else "—"
        lines.append(f"| {COUNTRY_FLAGS[code]} {COUNTRY_NAMES_EN[code]} (`{code}`) | {fmt(c.get('working', 0))} | {fmt(c.get('http', 0))} | {fmt(c.get('https', 0))} | {fmt(c.get('socks4', 0))} | {fmt(c.get('socks5', 0))} | {fmt(c.get('pac_http_https', 0))} | {fmt(c.get('socks_pac', 0))} | {latency_text} |")
